...
int main(int argc, char *argv[]) {
    ...
    file_p file;
    char *output, *input;
    unsigned long int tabelaCodigos[TAM_ASCII], input_length, chunck_length;
    int ocorrencias[TAM_ASCII], my_rank, n_procs;
    ...
    if(strcmp(argv[1], "1") == 0) {
        ...
        // Envia o tamanho do arquivo para todos
        MPI_Bcast(&input_length, 1, MPI_UNSIGNED, 0, MPI_COMM_WORLD);

        // Tamanho do chunck
        chunck_length = input_length / n_procs;

        // Se for o ultimo nodo
        if(rank == (n_procs-1)){
            chunck_length = input_length- ((n_procs-1) * chunck_length);   
        }
